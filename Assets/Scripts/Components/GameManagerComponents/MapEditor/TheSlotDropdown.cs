...
public class TheSlotDropdown : MonoBehaviour
{
    ...
    {
        GameObject hit_slot_object = null;
        if (hit_game_object != null && hit_game_object.CompareTag("Slot"))
        {
            hit_slot_object = hit_game_object;
        }
        if (hit_slot_object != null)
        {
            if (!this.gameObject.activeSelf && Input.GetMouseButtonUp((int)MouseButton.Left))
            {
                slot_gameobject = hit_slot_object;
    ...
    private void AddListenersToDropdown()
    {
        landscape_dropdown.onValueChanged.AddListener(
            (selected_index) =>
            {
                ...
                {
                    Slot current_slot = slot_gameobject.GetComponent<SlotComponent>().thisSlot;
                    current_slot.Landscape.LandscapeType = (LandscapeType)selected_index;
                }
            }
        );
        construction_dropdown.onValueChanged.AddListener(
            (selected_index) =>
            {
                ...
                {
                    Slot current_slot = slot_gameobject.GetComponent<SlotComponent>().thisSlot;
                    current_slot.Construction.ConstructionType = (ConstructionType)selected_index;
                }
            }
        );
        chess_dropdown.onValueChanged.AddListener(
            (selected_index) =>
            {
                ...
                {
                    Slot current_slot = slot_gameobject.GetComponent<SlotComponent>().thisSlot;
                    current_slot.Chess.ChessType = (ChessType)selected_index;
                }
            }
        );
    }
